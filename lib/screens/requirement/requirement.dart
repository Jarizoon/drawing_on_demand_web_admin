        ...
        child: AppLayout(
            content: FutureBuilder(
                ...
                builder: (context, snapshot) {
                  if (snapshot.hasData) {
                    List<Requirement> list = snapshot.data!.value.where((requirement) => requirement.title!.contains(search.toString())).where((requirement) => requirement.status!.contains(statusToFilter.toString())).where((requirement) => requirement.category!.name!.contains(categoryToFilter.toString())).toList();
                          ...
                          Container(
                            ...
                            child: Row(
                              ...
                              children: [
                                ...
                                Visibility(visible: MediaQuery.of(context).size.width >= 450, child: const Spacer(flex: 1)),
                                Visibility(
                                        ...
                                        children: [
                                          ...
                                          Expanded(
                                            ...
                                            child: DropdownButton<String?>(
                                                ...
                                                items: ["Pending", "Private", "Public", "None"].map<DropdownMenuItem<String?>>((e) => DropdownMenuItem(value: e, child: Text(e.toString()))).toList()),
                                          ),
                          ...
                          Container(
                              ...
                              child: Container(
                                ...
                                child: PaginatedDataTable(
                                  ...
                                  source: RequirementData(context: context, list: list),
                                  rowsPerPage: 10,
                                ),
                              )),
                        ],
...
class RequirementData extends DataTableSource {
  ...
  DataRow? getRow(int index) {
    ...
    return DataRow(onLongPress: () => context.goNamed(RequirementDetailRoute.name, pathParameters: {'requirement_id': requirement.id.toString()}), cells: [
      DataCell(Text("${requirement.title}", style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w600))),
      DataCell(Text("${requirement.createdByNavigation!.name}", style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w600))),
      DataCell(Text("${requirement.category!.name}", style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w600))),
      DataCell(Text("${requirement.budget}", style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w600))),
      DataCell(Text("${requirement.status}", style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w600)))
    ]);
  }
